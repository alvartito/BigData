...
public class MaponyMap extends Mapper<LongWritable, Text, Text, CustomWritable> {
	...
	protected void map(LongWritable offset, Text line, Context context) throws IOException, InterruptedException {
		...
		rdw = new RawDataWritable(dato[0], dato[1], dato[2], dato[3], dato[4], dato[5], dato[6], dato[7], dato[8], dato[9], dato[10], dato[11],
		...
		if ("1".toString().compareTo(rdw.getMarker()) != 0 && ("".compareTo(rdw.getLatitude()) != 0 && "".compareTo(rdw.getLongitude()) != 0)) {

			outKey = new Text(rdw.getIdentifier());

			CustomWritable cwDescripcion = new CustomWritable("descripcion");
			cwDescripcion.setTexto(rdw.getDescription());

			CustomWritable cwFoto = new CustomWritable("foto");
			String url = rdw.getDownloadUrl();
			cwFoto.setTexto(url);

			CustomWritable cwGeo = new CustomWritable("location");
			StringBuilder posicion = new StringBuilder();
			posicion.append(rdw.getLatitude());
			posicion.append(",");
			posicion.append(rdw.getLongitude());

			cwGeo.setTexto(posicion.toString());

			CustomWritable cwTags = new CustomWritable("tags");
			cwTags.setTexto(rdw.getMachineTags() + " " + rdw.getUserTags());

			CustomWritable cwTitulo = new CustomWritable("titulo");
			cwTitulo.setTexto(rdw.getTitle());
